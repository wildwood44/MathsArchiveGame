...
public class Puzzle_39 extends Puzzle {
	GamePanel gp;
	int question = 1;
	int bonus = 0;
	...
	public void generatePuzzle() {
		...
		values[0] = (Math.round(Math.random()*(5-1)))*10;
		double difference =  Math.round(Math.random()*(5-1));
		for(int i = 1; i < values.length; i++) {
			values[i] = Math.round(values[i-1] + ((values[i-1]/10) * difference));
			hidden[i] = false;
		}
