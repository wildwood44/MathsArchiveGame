...
public class Puzzle_24 extends Puzzle {
	GamePanel gp;
	int question = 1;
	...
	public Puzzle_24(GamePanel gp) {
		super(gp, "", SumType.MULTI, PuzzleType.FRACTION);
		this.gp = gp;
		generatePuzzle();
	}
	...
	public void generatePuzzle() {
		values = new double[5]; 
		hidden = new boolean[5];
		input = new String[5];
		hide = new int[2];
		values[0] = (Math.round(Math.random()*4)+1);
		double differnce = (Math.round(Math.random()*3)+1);
