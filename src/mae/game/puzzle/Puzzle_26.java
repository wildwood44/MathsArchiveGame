...
public class Puzzle_26 extends Puzzle {
	GamePanel gp;
	int question = 1;
	int bonus = 0;
	...
	public void generatePuzzle() {
		...
		double difference = Math.random() * (1 - 0.1) + 0.1;
		difference = (double)Math.round(difference*10)/10;
		for(int i = 1; i < values.length; i++) {
			values[i] = (double)Math.round((values[i-1] + difference)*10)/10;
			hidden[i] = false;
		}
	...
	public void drawPuzzle(Graphics2D g2) {
		...
		if(hidden[current]) {
			...
			g2.drawString(values[current]+"", gp.tileSize*5, gp.tileSize*3);
		}
